...
from pr_auto_reviewer.domain import (
    CommentId, CommitSha, Issue, IssueCreationError,
    ItemSeverity, PrComment, PullRequest, PullRequestId,
    PullRequestNotFoundError, ReviewItem,
)
...
class TestProcessIssueCommandsService:
    ...
    def _item(self):
        return ReviewItem(
            ...
            file_path="x.py", description="broken",
        )
    ...
    def test_processes_issue_command_and_creates_issues(self, _pr_id, _sha, _pr, _item):
        ...
        comment_reader = StubCommentReader([PrComment(
            id=CommentId(value="c1"), body="/create-issue 1",
            created_at=datetime(2025, 1, 1, tzinfo=timezone.utc),
        )])
        comment_publisher = StubCommentPublisher()
        tracker = StubIssueTracker([Issue(
            id=101, repository="owner/repo",
            title="[MAJOR] bug: broken", body="body",
            source_pr_id=_pr_id, source_item_number=1,
        )])

        svc = ProcessIssueCommandsService(
            pr_repo, review_reader, comment_reader, comment_publisher,
            tracker, ReviewItemParser(), IssueCommandParser(), IssueBodyBuilder(),
        )
    ...
    def test_noop_when_no_review(self, _pr_id, _sha, _pr):
        ...
        svc = ProcessIssueCommandsService(
            pr_repo, review_reader, StubCommentReader(), StubCommentPublisher(),
            tracker, ReviewItemParser(), IssueCommandParser(), IssueBodyBuilder(),
        )
        svc.execute(ProcessIssueCommandsCommand(pr_id=_pr_id, head_sha=_sha))
        assert len(tracker.create_calls) == 0
    ...
    def test_noop_when_no_comments(self, _pr_id, _sha, _pr):
        ...
        svc = ProcessIssueCommandsService(
            pr_repo, review_reader, comment_reader, StubCommentPublisher(),
            tracker, ReviewItemParser(), IssueCommandParser(), IssueBodyBuilder(),
        )
        svc.execute(ProcessIssueCommandsCommand(pr_id=_pr_id, head_sha=_sha))
        assert len(tracker.create_calls) == 0
    ...
    def test_raises_when_pr_not_found(self, _pr_id, _sha):
        ...
        svc = ProcessIssueCommandsService(
            pr_repo, StubReviewReader(), StubCommentReader(),
            StubCommentPublisher(), StubIssueTracker(),
            ReviewItemParser(), IssueCommandParser(), IssueBodyBuilder(),
        )
