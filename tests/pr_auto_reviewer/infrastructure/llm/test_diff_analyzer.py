...
@pytest.fixture
def sample_diff():
    # one modified file with a hunk starting at line 2 and 3 changed lines
    return (
...
def large_file_contents():
    # 200 lines to test truncation and context extraction
...
def test_parse_diff_hunks_and_extract_context(sample_diff, large_file_contents):
    ...
    # hunk started at +2 and covered 3 lines (+addedA,+addedB,-removed) -> count should be 3
    assert hunks["foo.txt"][0][0] == 2
    assert hunks["foo.txt"][0][1] == 4

    # extract context around that hunk from a large file and ensure line numbers are present
    context = da.extract_context_around_hunks(large_file_contents, hunks["foo.txt"], context_lines=2, max_chars=500)
    assert "    1:" in context or "1:" in context  # line numbering present
...
def test_trim_diff_by_file_boundaries_success_and_failure(sample_diff):
    da = DiffAnalyzer()
    raw = sample_diff * 10  # make it larger

    # case: budget allows the diff -> should return identical string
    result_ok = da.trim_diff_by_file_boundaries(raw, max_tokens=10000)
    assert result_ok == raw

    # case: very small budget -> trimmed output should be shorter and not equal to raw
...
def test_trim_file_contents_to_limits(sample_diff, large_file_contents):
    ...
    # foo.txt should be truncated because its content is large
