...
- Returning `Result<(), Box<dyn Error>>` in library code instead of a concrete error type
...
pub fn load_config(path: &str) -> Result<Config, String> {
    let data = std::fs::read_to_string(path).unwrap();  // Panics on error
    let config: Config = match serde_json::from_str(&data) {
        Ok(c) => c,
        Err(_) => return Err("parse error".to_string()),  // Lost context
    };
    Ok(config)
}
```
